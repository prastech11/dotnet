pipeline {
  ...
  stages {
    ...
    stage('Copyfilesfrom S3') {
      steps {
        s3Download(file: 'deployment.yaml', bucket: 's3://sam-bucket-jnks', pathStyleAccessEnabled: true)
        s3Download(file: 'service.yaml', bucket: 's3://sam-bucket-jnks', pathStyleAccessEnabled: true)
      }
    }
